...
//  Hike
//
//  Created by Oğuzhan Cnr on 21.10.2024.
//
...
struct SettingsView: View {
    ...
    private let alterneteAppIcons: [String] = [
    ...
    var body: some View {
        List {
            ...
            Section(header: Text("Alterne Icons")) {
                ScrollView(.horizontal, showsIndicators:
                            false) {
                    HStack (spacing: 12) {
                        ForEach(alterneteAppIcons.indices, id: \.self) {
                            item in
                            Button{
                                print("Icon \(alterneteAppIcons[item]) was pressed.")
                                
                                UIApplication.shared.setAlternateIconName(alterneteAppIcons[item]) { error in
                                    if error != nil {
            print("Failed request to update the app's icon:\(String(describing: error?.localizedDescription))")
                                    }
                                    else {
            print("Successfully updated the app's icon to \(alterneteAppIcons[item])")
                                    }
                                }
                                    
                            } label: {
                                Image("\(alterneteAppIcons[item])-Preview")
                                    .resizable()
                                    .scaledToFit()
            ...
            Section(
                ...
                CustomListRow(rowLabel: "Version", rowIcon: "gear", rowContent: "1.0", rowTintColor: .gray)
                
                CustomListRow(rowLabel: "Developer", rowIcon: "ellipsis.curlybraces", rowContent: "Oğuzhan Çınar", rowTintColor: .mint)
